...
public:
	...
	void solveNbody(float& accumulator, std::vector<T>& particles, std::vector<int>& indexTracker)
	{
		...
		for (int i = 0; i < particles.size(); i++)
		{
			for (int j = 0; j < particles.size(); j++)
			{
				if (i != j)
				{
					...
					kernel(accumulator, particles[i], particles[j]);
	...
	void updateTree(std::vector<T>& particles, std::vector<int>& indexTracker)
	{
